            ...
            <div class="columns is-12">
                <div class="column left">
                    ...
                    <nav class="level properties">
                        <div class="level-item" v-for="post in posts" :key="post_id">
                            <div class="card">
                        ...
                        <span>
                          ...
                          <i class="fa fa-arrows-alt" aria-hidden="true"></i>100m&#178;
                                            ...
                                            <div>
                                                <small>{{post.ward}}, {{post.district}}, {{post.province}}</small>
                                            </div>
                                            <div>
                    ...
                    <nav class="pagination is-centered">
                        <ul class="pagination-list">
                            <li v-if="pageCurrents > 1">
                                <a class="pagination-link" :href="`/for_sale/page/${pageCurrents - 1}`">Prev</a>
                            </li>
                            <li v-for="i in (1, allPages)">
                                <a v-if="pageCurrents == i" class="pagination-link is-current"
                                   :href="`/for_sale/page/${i}`">{{i}}</a>
                                <a v-else class="pagination-link" :href="`/for_sale/page/${i}`">{{i}}</a>
                            </li>
                            <li v-if="pageCurrents < allPages">
                                <a class="pagination-link" :href="`/for_sale/page/${pageCurrents + 1}`">Next</a>
                            </li>
                        </ul>
                    </nav>
                </div>
...
<script>
  ...
  export default {
    ...
    async asyncData ({params}) {
      ...
      // let dataPost = await axios.get(`/api/posts/sw`)
      console.log(dataPost.data)
      return {
        posts: dataPost.data.posts,
        countAlls: dataPost.data.countAll,
        allPages: dataPost.data.allpage,
        pageCurrents: dataPost.data.pageCurrent
      }
    },
    data () {
      return {
